...
/** User-selected local audio plus direct Yandex Music quiz fragments. */
public final class PartyMusic {
 ...
 private final Context context;private final YandexMusicClient yandex;private MediaPlayer player;private final Handler handler=new Handler(Looper.getMainLooper());
 ...
 private void emit(String s){status=s;if(listener!=null)listener.onState(s);DiagnosticRecorder.get(context).event("party_music",s);}
 ...
 public void ensureBackground(){if(tracks.isEmpty()||player!=null||yandex.isPlaying()||!context.getSharedPreferences("martin",0).getBoolean("auto_music",true))return;requestedVolume=.28f;Track t=tracks.get(index%tracks.size());open(t.uri,"Фон: "+t.artist+" — "+t.title,0,null,true);}
 ...
 public boolean hasYandexAccount(){return yandex.hasToken();}
 public void openYandexLogin(){try{Intent i=new Intent(context,YandexAuthActivity.class);i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);context.startActivity(i);}catch(Exception e){emit("Не удалось открыть вход в Яндекс Музыку");}}

 public void clip(String uri,Runnable done){
  if(isYandexClipUri(uri)){
   stopLocalOnly();clipDone=done;clipMode=true;yandexClip=true;backgroundMode=false;requestedVolume=1f;String query=uri.substring("yandex:".length()).trim();
   if(!yandex.hasToken()){emit("Подключите Яндекс Музыку один раз в разделе «Музыка»");finishClip();return;}
   emit("Готовлю фрагмент из Яндекс Музыки");yandex.setVolume(1f);
   yandex.playFragment(query,6,new YandexMusicClient.Callback(){
    public void onStarted(YandexMusicClient.TrackInfo t){emit("Фрагмент: "+t.label());}
    public void onFinished(YandexMusicClient.TrackInfo t){if(clipMode&&yandexClip)finishClip();}
    public void onError(String e){emit("Яндекс Музыка: "+e);if(clipMode&&yandexClip)finishClip();}
   });return;
  }
 ...
 private void open(String uri,String label,int duration,Runnable done,boolean background){
  stopLocalOnly();clipDone=done;clipMode=duration>0;backgroundMode=background;yandexClip=false;final MediaPlayer p=new MediaPlayer();player=p;
 ...
 private void finishClip(){Runnable d=clipDone;clipDone=null;clipMode=false;yandexClip=false;if(player!=null)releaseLocal();if(d!=null)d.run();}
 private void releaseLocal(){handler.removeCallbacksAndMessages(null);if(player!=null){try{player.release();}catch(Exception ignored){}player=null;}}
 private void stopLocalOnly(){handler.removeCallbacksAndMessages(null);clipDone=null;clipMode=false;backgroundMode=false;if(player!=null){try{player.stop();}catch(Exception ignored){}releaseLocal();}}
 public void stopClip(){if(clipMode){if(yandexClip)yandex.stop();stopLocalOnly();emit("Фрагмент остановлен");}}
 public void stop(){yandex.stop();stopLocalOnly();yandexClip=false;emit("Музыка остановлена");}
}
