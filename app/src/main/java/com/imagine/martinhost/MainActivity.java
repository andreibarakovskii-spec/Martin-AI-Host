...
public final class MainActivity extends Activity implements RecognitionListener, TextToSpeech.OnInitListener {
    ...
    @Override public void onResults(Bundle b){ArrayList<String> r=b.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION);String t=r==null||r.isEmpty()?"":r.get(0).trim();lastHeard.setText(t);String low=t.toLowerCase(Locale.ROOT);if(low.contains("мартин стоп")||low.equals("стоп")){hardStop();return;}
        if(awaitingScoreDelta>0){int delta=awaitingScoreDelta;if(guests.addScore(t,delta)){awaitingScoreDelta=0;speakLocal("Записал. "+t+" получает "+delta+" балл!",MartinCharacterView.State.HAPPY);}else{speakLocal("Не нашёл «"+t+"» в списке гостей. Назовите имя ещё раз.",MartinCharacterView.State.LISTENING);}return;}
        int x=low.indexOf("мартин");if(x>=0){String q=t.substring(Math.min(t.length(),x+6)).replaceFirst("^[,.:;!?\\s-]+","");if(q.isBlank())q="Поздоровайся и спроси, чем помочь компании.";String ql=q.toLowerCase(Locale.ROOT);if(ql.contains("следующ")&&(ql.contains("игр")||ql.contains("конкурс"))){startNextGame();return;}ask(q);}else if(active&&!paused)startListening();}
