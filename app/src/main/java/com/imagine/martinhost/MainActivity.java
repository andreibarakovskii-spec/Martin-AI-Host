...
public final class MainActivity extends Activity implements RecognitionListener, TextToSpeech.OnInitListener {
    ...
    private MartinSpriteView martin; private TextView state,lastHeard,lastReply,providerBadge;
    ...
    private void startListening(){if(paused||speaking||!active)return;if(checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED){requestPerms();return;}if(sr!=null)try{sr.destroy();}catch(Exception ignored){}sr=SpeechRecognizer.isOnDeviceRecognitionAvailable(this)?SpeechRecognizer.createOnDeviceSpeechRecognizer(this):SpeechRecognizer.createSpeechRecognizer(this);sr.setRecognitionListener(this);Intent i=new Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH);i.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL,RecognizerIntent.LANGUAGE_MODEL_FREE_FORM);i.putExtra(RecognizerIntent.EXTRA_LANGUAGE,"ru-RU");i.putExtra(RecognizerIntent.EXTRA_PARTIAL_RESULTS,true);i.putExtra(RecognizerIntent.EXTRA_PREFER_OFFLINE,true);sr.startListening(i);if(awaitingScoreDelta>0)setMartin(MartinSpriteView.State.LISTENING,"Назовите имя ответившего");else setMartin(MartinSpriteView.State.LISTENING,"Слушаю тебя…");}

    private MartinSpriteView.State stateForPrompt(String q){String s=q.toLowerCase(Locale.ROOT);if(s.contains("тост"))return MartinSpriteView.State.TOAST;if(s.contains("музык")||s.contains("песн")||s.contains("мелоди")||s.contains("dj")||s.contains("дидже"))return MartinSpriteView.State.DJ;if(s.contains("конкурс")||s.contains("игр")||s.contains("виктор")||s.contains("что? где? когда")||s.contains("цитат"))return MartinSpriteView.State.GAME;return MartinSpriteView.State.TALKING;}

    private void speak(String text,MartinSpriteView.State visual){if(text==null||text.isBlank()){speaking=false;startListening();return;}speaking=true;if(sr!=null)try{sr.cancel();}catch(Exception ignored){}lastReply.setText(text);setMartin(visual,"Говорю…");if(!ttsReady){lastReply.setText(text+"\n\n⚠ Голос Android ещё не готов. Открой настройки и нажми «Проверить голос».");speaking=false;return;}int r=tts.speak(text,TextToSpeech.QUEUE_FLUSH,null,"martin");if(r==TextToSpeech.ERROR){speaking=false;state.setText("Ошибка озвучки Android TTS");}}
    private void ask(String q){speaking=true;if(sr!=null)try{sr.cancel();}catch(Exception ignored){}setMartin(MartinSpriteView.State.THINKING,"Думаю…");final MartinSpriteView.State replyState=stateForPrompt(q);grok.reply(q,new GrokClient.Callback(){public void onResult(String text){runOnUiThread(()->{String marker="[[ASK_NAME_FOR_SCORE:1]]";boolean score=text.contains(marker);String clean=text.replace(marker,"").trim();if(score)awaitingScoreDelta=1;speak(clean,replyState);});}public void onError(String e){runOnUiThread(()->{speaking=false;martin.setState(MartinSpriteView.State.IDLE);state.setText("AI не ответил");lastReply.setText(e);if(active&&!paused)startListening();});}});}
    ...
    private void hardStop(){active=false;paused=false;speaking=false;awaitingScoreDelta=0;if(sr!=null)try{sr.cancel();}catch(Exception ignored){}if(tts!=null)tts.stop();setMartin(MartinSpriteView.State.SLEEPING,"Остановлено");main.setText("НАЧАТЬ СЛУШАТЬ");}

    @Override public void onInit(int s){if(s==TextToSpeech.SUCCESS){int lang=tts.setLanguage(new Locale("ru","RU"));ttsReady=lang!=TextToSpeech.LANG_MISSING_DATA&&lang!=TextToSpeech.LANG_NOT_SUPPORTED;tts.setSpeechRate(1.02f);tts.setPitch(.98f);tts.setOnUtteranceProgressListener(new android.speech.tts.UtteranceProgressListener(){public void onStart(String id){runOnUiThread(()->martin.setSpeechLevel(.7f));}public void onDone(String id){runOnUiThread(()->{martin.setSpeechLevel(0f);speaking=false;martin.setState(MartinSpriteView.State.HAPPY);new android.os.Handler(getMainLooper()).postDelayed(()->{if(active&&!paused)startListening();else martin.setState(MartinSpriteView.State.IDLE);},450);});}public void onError(String id){runOnUiThread(()->{martin.setSpeechLevel(0f);speaking=false;state.setText("Ошибка TTS");if(active&&!paused)startListening();});}});}else{ttsReady=false;state.setText("Android TTS недоступен");}}

    @Override public void onResults(Bundle b){ArrayList<String> r=b.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION);String t=r==null||r.isEmpty()?"":r.get(0).trim();lastHeard.setText(t);String low=t.toLowerCase(Locale.ROOT);if(low.contains("мартин стоп")||low.equals("стоп")){hardStop();return;}if(awaitingScoreDelta>0){int delta=awaitingScoreDelta;if(guests.addScore(t,delta)){awaitingScoreDelta=0;speak("Записал. "+t+" получает "+delta+" балл!",MartinSpriteView.State.HAPPY);}else{speak("Не нашёл «"+t+"» в списке гостей. Назови имя ещё раз.",MartinSpriteView.State.LISTENING);}return;}int x=low.indexOf("мартин");if(x>=0){String q=t.substring(Math.min(t.length(),x+6)).replaceFirst("^[,.:;!?\\s-]+","");if(q.isBlank())q="Поздоровайся и спроси, чем помочь компании.";String ql=q.toLowerCase(Locale.ROOT);if(ql.contains("следующ")&&(ql.contains("игр")||ql.contains("конкурс"))){startNextGame();return;}ask(q);}else if(active&&!paused)startListening();}
    @Override public void onPartialResults(Bundle b){} @Override public void onReadyForSpeech(Bundle b){martin.setState(MartinSpriteView.State.LISTENING);} @Override public void onBeginningOfSpeech(){setMartin(MartinSpriteView.State.LISTENING,"Слышу тебя…");} @Override public void onRmsChanged(float v){if(!speaking)martin.setSpeechLevel(Math.max(0f,Math.min(1f,(v+2f)/12f)));} @Override public void onBufferReceived(byte[] b){} @Override public void onEndOfSpeech(){setMartin(MartinSpriteView.State.THINKING,"Разбираю фразу…");} @Override public void onEvent(int t,Bundle b){} @Override public void onError(int e){if(active&&!paused&&!speaking)new android.os.Handler(getMainLooper()).postDelayed(this::startListening,350);}
    ...
    private void buildUi(){getWindow().setStatusBarColor(0xFF07080D);getWindow().setNavigationBarColor(0xFF07080D);ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(18),dp(16),dp(18),dp(30));root.setBackgroundColor(0xFF07080D);scroll.addView(root);
        ...
        main.setOnClickListener(v->{active=!active;if(active){paused=false;main.setText("●  СЛУШАЮ");startListening();}else hardStop();});pause.setOnClickListener(v->{paused=!paused;if(paused){if(sr!=null)sr.cancel();if(tts!=null)tts.stop();setMartin(MartinSpriteView.State.SLEEPING,"Пауза");}else{setMartin(MartinSpriteView.State.IDLE,"Возобновляю…");startListening();}});stop.setOnClickListener(v->hardStop());next.setOnClickListener(v->startNextGame());settings.setOnClickListener(v->startActivity(new Intent(this,SettingsActivity.class)));testVoice.setOnClickListener(v->speak("Привет! Я Мартин. Голос работает, и я готов вести вечеринку.",MartinSpriteView.State.HAPPY));refreshProviderBadge();}
