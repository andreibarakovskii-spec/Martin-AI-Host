...
/** Party-first launcher with the real embedded Godot 3D Martin NPC. */
public final class PremiumMainActivity extends FragmentActivity implements GodotHost {
    ...
    @Override public void onCreate(Bundle b) {
            ...
            @Override public void onSpeechChunk(byte[] wav) {
                ...
                stt.transcribe(wav, new GroqTranscriber.Callback() {
                    ...
                    @Override public void onError(String e) { runOnUiThread(() -> {
                        ...
                        subline.setText("Попробуй сказать ещё раз");
                        turns.forceListen();
                    }); }
                });
            }
            ...
            @Override public void onLevel(float rms, float noise, boolean speech) {
                runOnUiThread(() -> {
                    if (turns.acceptMicForStt()) {
                        setMartinSpeech(Math.max(0f, Math.min(1f, (rms + 48f) / 36f)));
                    }
                });
            }
        ...
        neural = new MartinNeuralSpeaker(this, new MartinNeuralSpeaker.Listener() {
            ...
            @Override public void onDone() { runOnUiThread(() -> {
                setMartinSpeech(0f);
                setMartinState("happy");
                if (martinPlugin != null) martinPlugin.triggerAction("cheer");
                turns.onAiSpeechDone();
            }); }
            @Override public void onError(String message) { runOnUiThread(() -> {
                neuralReady = false;
                setVoiceDot(false);
                state.setText("Голос не установлен");
                subline.setText("Скачай модель в настройках");
                turns.onAiSpeechDone();
            }); }
        });
    ...
    private void buildUi() {
        ...
        TextView tag = text("AI HOST • 3D NPC", 10, 0xFF9B5CFF, Typeface.BOLD);
        tag.setLetterSpacing(.14f);
        ...
        statusRow.addView(aiDot, new LinearLayout.LayoutParams(dp(82), dp(28)));
        statusRow.addView(new Space(this), new LinearLayout.LayoutParams(dp(10), 1));
        statusRow.addView(voiceDot, new LinearLayout.LayoutParams(dp(106), dp(28)));
        root.addView(statusRow);

        FrameLayout hero = new FrameLayout(this);
        hero.setBackground(gradient(0xFF070812, 0xFF130D24, 0xFF070811, 28));
        godotContainer = new FrameLayout(this);
        godotContainer.setId(View.generateViewId());
        hero.addView(godotContainer, new FrameLayout.LayoutParams(-1, -1));

        TextView badge = text("RIGGED 3D • LIVE", 9, 0xFFB68CFF, Typeface.BOLD);
        badge.setGravity(Gravity.CENTER);
        badge.setBackground(round(0x991A1030, 14));
        FrameLayout.LayoutParams bp = new FrameLayout.LayoutParams(dp(118), dp(26), Gravity.TOP | Gravity.RIGHT);
        bp.setMargins(0, dp(10), dp(10), 0);
        hero.addView(badge, bp);
        root.addView(hero, new LinearLayout.LayoutParams(-1, 0, 1));

        state = text("Готов к вечеринке", 20, Color.WHITE, Typeface.BOLD);
        state.setGravity(Gravity.CENTER);
        state.setPadding(0, dp(8), 0, 0);
        root.addView(state);

        subline = text("Скажи: «Мартин, привет»", 12, 0xFF8D8997, Typeface.NORMAL);
    ...
    private void attachGodotRenderer() {
        ...
        if (existing instanceof GodotFragment) {
            ...
            getSupportFragmentManager().beginTransaction()
                    .replace(godotContainer.getId(), godotFragment, "martin_godot")
    ...
    @Override public Set<GodotPlugin> getHostPlugins(Godot godot) {
        ...
        godotContainer.postDelayed(this::syncMartin, 900);
        return Set.of(martinPlugin);
    }

    private void syncMartin() {
        if (martinPlugin == null) return;
        martinPlugin.setState(pendingMartinState);
        martinPlugin.setSpeechLevel(pendingMartinSpeech);
    }

    private void setMartinState(String value) {
        pendingMartinState = value == null ? "idle" : value;
        if (martinPlugin != null) martinPlugin.setState(pendingMartinState);
    }

    private void setMartinSpeech(float value) {
        pendingMartinSpeech = Math.max(0f, Math.min(1f, value));
        if (martinPlugin != null) martinPlugin.setSpeechLevel(pendingMartinSpeech);
    }
    ...
    private void onTurnState(TurnManager.State s) {
        ...
        switch (s) {
            case LISTENING:
                setMartinState("listening");
                state.setText("Слушаю…");
                subline.setText("Я слышу компанию");
                break;
            case THINKING:
                setMartinState("thinking");
                state.setText("Думаю…");
                subline.setText("Секунду, формулирую ответ");
                break;
            case SPEAKING:
                setMartinState("talking");
                state.setText("Говорю…");
                subline.setText("Мартин отвечает");
                break;
            case COOLDOWN:
                state.setText("Слушаю…");
                break;
    ...
    private void stopAudio() {
        ...
        subline.setText("Скажи: «Мартин, привет»");
        mic.setText("🎙  НАЧАТЬ");
    }
    ...
    private void ask(String q) {
        ...
        grok.reply(q, new GrokClient.Callback() {
            ...
            @Override public void onError(String e) { runOnUiThread(() -> {
                ...
                subline.setText("Проверь Groq / GSK в настройках");
                setMartinState("listening");
                turns.forceListen();
            }); }
        });
    ...
    private void refreshStatus() {
        ...
        setVoiceDot(getSharedPreferences("martin", 0).getBoolean("voice_model_ready", false));
    }
    ...
    @Override public void onRequestPermissionsResult(int r, String[] p, int[] g) {
        ...
        if (r == REQ && active) startAudio();
    }

    @Override protected void onResume() {
        super.onResume();
        refreshStatus();
        if (neural != null && !neuralReady && getSharedPreferences("martin", 0).getBoolean("voice_model_ready", false)) neural.prepare();
        godotContainer.postDelayed(this::syncMartin, 500);
    }
