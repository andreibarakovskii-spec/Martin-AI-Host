...
/** Party-first launcher using the real-time OpenGL Martin NPC. */
public final class PremiumMainActivity extends Activity {
        ...
        audio=new ContinuousSpeechEngine(this,turns,new ContinuousSpeechEngine.Listener(){
            @Override public void onSpeechChunk(byte[] wav){
                ...
                stt.transcribe(wav,new GroqTranscriber.Callback(){
                    ...
                    @Override public void onError(String e){runOnUiThread(()->{state.setText("Не расслышал");subline.setText("Попробуй сказать ещё раз");turns.forceListen();});}
                });
            }
            ...
            @Override public void onError(String e){runOnUiThread(()->subline.setText("Нужен доступ к микрофону"));}
        });
        neural=new MartinNeuralSpeaker(this,new MartinNeuralSpeaker.Listener(){
            @Override public void onPreparing(String m){runOnUiThread(()->setVoiceDot(false));}
            @Override public void onReady(){runOnUiThread(()->{neuralReady=true;setVoiceDot(true);});}
            @Override public void onStart(){runOnUiThread(()->{martin.setState(Martin3DView.State.TALKING);state.setText("Говорю…");});}
            @Override public void onLevel(float level){runOnUiThread(()->martin.setSpeechLevel(level));}
            @Override public void onDone(){runOnUiThread(()->{martin.setSpeechLevel(0f);martin.setState(Martin3DView.State.HAPPY);turns.onAiSpeechDone();});}
            @Override public void onError(String message){runOnUiThread(()->{neuralReady=false;setVoiceDot(false);state.setText("Голос не установлен");subline.setText("Скачай модель в настройках");turns.onAiSpeechDone();});}
        });
    ...
    private void buildUi(){
        ...
        root.setPadding(dp(16),dp(7),dp(16),dp(10));
        ...
        statusRow.addView(aiDot,new LinearLayout.LayoutParams(dp(82),dp(28)));statusRow.addView(new Space(this),new LinearLayout.LayoutParams(dp(10),1));statusRow.addView(voiceDot,new LinearLayout.LayoutParams(dp(106),dp(28)));root.addView(statusRow);

        FrameLayout hero=new FrameLayout(this);hero.setBackground(gradient(0xFF0A0B12,0xFF151027,0xFF080910,28));
        martin=new Martin3DView(this);hero.addView(martin,new FrameLayout.LayoutParams(-1,-1));
        TextView badge=text("3D NPC • LIVE",9,0xFFB68CFF,Typeface.BOLD);badge.setGravity(Gravity.CENTER);badge.setBackground(round(0x991A1030,14));FrameLayout.LayoutParams bp=new FrameLayout.LayoutParams(dp(104),dp(26),Gravity.TOP|Gravity.RIGHT);bp.setMargins(0,dp(10),dp(10),0);hero.addView(badge,bp);
    ...
    private void addNav(LinearLayout nav,String icon,String label,boolean selected,View.OnClickListener click){
        LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setGravity(Gravity.CENTER);box.setPadding(0,dp(3),0,dp(2));if(selected)box.setBackground(round(0xFF20163A,18));
        TextView i=text(icon,17,selected?0xFF9D63FF:0xFFE5E2EC,Typeface.NORMAL);i.setGravity(Gravity.CENTER);
        TextView l=text(label,9,selected?0xFFB987FF:0xFFE5E2EC,selected?Typeface.BOLD:Typeface.NORMAL);l.setGravity(Gravity.CENTER);
        box.addView(i);box.addView(l);if(click!=null)box.setOnClickListener(click);nav.addView(box,new LinearLayout.LayoutParams(0,-1,1));
    }

    private void onTurnState(TurnManager.State s){
        if(!active)return;
        switch(s){
            case LISTENING: martin.setState(Martin3DView.State.LISTENING);state.setText("Слушаю…");subline.setText("Я слышу компанию");break;
            case THINKING: martin.setState(Martin3DView.State.THINKING);state.setText("Думаю…");subline.setText("Секунду, формулирую ответ");break;
            case SPEAKING: martin.setState(Martin3DView.State.TALKING);state.setText("Говорю…");subline.setText("Мартин отвечает");break;
    ...
    private void startAudio(){
        ...
        active=true;turns.forceListen();audio.start();martin.setState(Martin3DView.State.LISTENING);state.setText("Слушаю…");subline.setText("Скажи: «Мартин…»");mic.setText("●  СЛУШАЮ");
    }
    private void stopAudio(){active=false;audio.stop();if(neural!=null)neural.stop();turns.forceListen();martin.setState(Martin3DView.State.IDLE);state.setText("Готов к вечеринке");subline.setText("Скажи: «Мартин, привет»");mic.setText("🎙  НАЧАТЬ");}

    private void handleTranscript(String t){
        if(t==null||t.isBlank()){turns.forceListen();return;}heard.setText("Вы: "+t);
        String low=t.toLowerCase(Locale.ROOT);if(low.contains("мартин стоп")||low.equals("стоп")){stopAudio();return;}
        int x=low.indexOf("мартин");if(x<0){turns.forceListen();return;}
        String q=t.substring(Math.min(t.length(),x+6)).replaceFirst("^[,.:;!?\\s-]+","");if(q.isBlank())q="Поздоровайся с компанией естественно и коротко.";ask(q);
    }

    private void ask(String q){
        turns.onUserFinal();martin.setState(Martin3DView.State.THINKING);state.setText("Думаю…");subline.setText("Секунду, формулирую ответ");
        grok.reply(q,new GrokClient.Callback(){
            @Override public void onResult(String raw){runOnUiThread(()->speak(cleanSpeech(raw)));}
            @Override public void onError(String e){runOnUiThread(()->{state.setText("Нет связи с AI");subline.setText("Проверь Groq / GSK в настройках");turns.forceListen();});}
        });
    }

    private void speak(String text){
        if(text.isBlank()){turns.forceListen();return;}reply.setText(text);turns.onAiWillSpeak();martin.setState(Martin3DView.State.TALKING);state.setText("Говорю…");subline.setText("Мартин отвечает");
        if(neuralReady&&neural.isReady())neural.speak(text,"playful",.65f);else{state.setText("Голос не установлен");subline.setText("Скачай модель в настройках");turns.onAiSpeechDone();}
    }
    ...
    private void refreshStatus(){String key=getSharedPreferences("martin",0).getString("ai_key","");long ok=getSharedPreferences("martin",0).getLong("groq_last_ok",0);boolean ai=key.startsWith("gsk_")&&ok>0;aiDot.setText(ai?"●  AI":"●  AI?");aiDot.setTextColor(ai?0xFF58E6A9:0xFFFFB35C);setVoiceDot(getSharedPreferences("martin",0).getBoolean("voice_model_ready",false));}
    ...
    @Override protected void onResume(){super.onResume();if(martin!=null)martin.onResume();refreshStatus();if(neural!=null&&!neuralReady&&getSharedPreferences("martin",0).getBoolean("voice_model_ready",false))neural.prepare();}
    @Override protected void onPause(){if(martin!=null)martin.onPause();super.onPause();}
