...
public final class LocalVoicesActivity extends Activity {
 ...
 @Override public void onCreate(Bundle b){super.onCreate(b);prefs=getSharedPreferences("martin",0);LinearLayout root=PartyScreen.root(this,"Бесплатные голоса");
  PartyScreen.text(this,root,"5 мужских и 5 женских голосов Supertonic. Без подписок, ключей и платы за синтез. Одна модель около 380 МБ на все голоса; интернет нужен только для её загрузки. Само распознавание речи и ответы AI пока используют ваш текущий API.",15);
  PartyScreen.text(this,root,"Это другие тембры той же модели, а не новый синтезатор. Сравните произношение и интонацию на телефоне. Прежний голос — Женский 1 / F1.",15);
  ...
  for(String id:LocalVoiceProfiles.IDS){
   ...
   PartyScreen.button(this,root,"✓ Оставить "+id+" для Мартина",()->{player.stop();prefs.edit().putString("local_voice",id).putFloat("local_voice_speed",rate()).apply();refreshChosen();ui("Сохранено. На главном экране Мартин будет говорить выбранным голосом.");});
  }
 ...
 private void refreshSpeed(){speedLabel.setText(String.format(Locale.forLanguageTag("ru"),"Темп воспроизведения: %.2f× (высота голоса сохраняется)",rate()));}
