...
func _ready()->void:
    ...
    if packed==null:push_error("MARTIN_PRODUCTION_MODEL_MISSING %s"%MODEL_PATH);return
    model_instance=packed.instantiate() as Node3D
    if model_instance==null:push_error("MARTIN_PRODUCTION_MODEL_INVALID");return
    model_instance.name="MartinRiggedProduction"
    model_mount.add_child(model_instance)
    _fit_model_to_stage()
    model_instance.visible=true

    adapter=AvatarModelAdapter.new();adapter.name="AvatarModelAdapter";adapter.model_root_path=NodePath("../ModelMount");add_child(adapter)
    await get_tree().process_frame
    if adapter==null or not adapter.is_production_model_ready():push_error("MARTIN_PRODUCTION_NO_SKELETON");return
...
func _on_host_speech(v:float)->void:MartinBridge.set_speech_level(v)
func _on_host_action(v:String)->void:MartinBridge.trigger_action(v)
func _on_host_look(x:float,y:float)->void:MartinBridge.set_look(x,y)
func _on_host_energy(v:float)->void:MartinBridge.set_energy(v)
func _on_host_emotion(v:String)->void:MartinBridge.set_emotion(v)

func _process(delta:float)->void:
    if model_instance==null:return
    elapsed+=delta;blink_clock+=delta
    var idle_sway:float=sin(elapsed*1.15)*0.004
    var performance_sway:=0.0
    if current_state=="talking":performance_sway=sin(elapsed*5.2)*(0.002+speech_level*0.006)
    elif current_state=="dj":performance_sway=sin(elapsed*3.4)*0.012
    ...
    if adapter!=null:
        if current_state=="talking":adapter.drive_simple_lipsync(speech_level)
        if blink_clock>=next_blink:_blink();blink_clock=0.0;next_blink=randf_range(2.7,5.1)

func _fit_model_to_stage()->void:
    var bounds:AABB=_collect_bounds(model_instance)
    if bounds.size.y<=0.001:push_error("MARTIN_PRODUCTION_EMPTY_BOUNDS");return
    var factor:float=2.55/bounds.size.y;model_instance.scale=Vector3.ONE*factor
...
func _collect_bounds(root:Node3D)->AABB:
    var result:=AABB();var initialized:=false;var stack:Array[Node]=[root]
    while not stack.is_empty():
        var node:Node=stack.pop_back() as Node
        if node is MeshInstance3D:
            var m:=node as MeshInstance3D
            if m.mesh!=null:
                var box:AABB=m.get_aabb();var rel:Transform3D=root.global_transform.affine_inverse()*m.global_transform
                ...
                for corner in corners:
                    ...
                    if not initialized:result=AABB(p,Vector3.ZERO);initialized=true
                    else:result=result.expand(p)
        for child in node.get_children():stack.push_back(child as Node)
    return result

func _on_state_changed(value:String)->void:
    current_state=value.to_lower();if adapter==null:return
    adapter.clear_face();adapter.travel(current_state)
    if current_state=="talking":adapter.set_expression("brow_up",0.10+energy*0.08)
    elif current_state=="happy":adapter.set_expression("smile",0.65)
    elif current_state=="thinking":adapter.set_expression("brow_up",0.28)
...
func _blink()->void:
    if adapter==null:return
    var tween:Tween=create_tween();tween.set_parallel(true);tween.tween_method(func(v:float):adapter.set_expression("blink_l",v),0.0,1.0,0.065);tween.tween_method(func(v:float):adapter.set_expression("blink_r",v),0.0,1.0,0.065)
    await tween.finished
    var open_tween:Tween=create_tween();open_tween.set_parallel(true);open_tween.tween_method(func(v:float):adapter.set_expression("blink_l",v),1.0,0.0,0.080);open_tween.tween_method(func(v:float):adapter.set_expression("blink_r",v),1.0,0.0,0.080)
func _run_smoke()->void:
    await get_tree().create_timer(0.25).timeout
    var caps:Dictionary=adapter.get_capabilities() if adapter!=null else {}
    if not caps.get("skeleton",false):push_error("MARTIN_SMOKE_FAIL skeleton");get_tree().quit(2);return
    if not caps.get("animation_player",false):push_error("MARTIN_SMOKE_FAIL animation_player");get_tree().quit(3);return
